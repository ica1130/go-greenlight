...
# Prompt to enter a password for the PostgreSQL greenlight user (rather than harcoding
# a password in this script).
read -p "Enter password for greenlight DB user: " DB_PASSWORD

# Force all output to be presented in the en_US for the duration of this script. This avoids
...
# files will be replaced if the newer ones are available.
apt update
...
rsync --archive --chown=${USERNAME}:${USERNAME} /c/Users/ppomi/.ssh /home/${USERNAME}

# Configurate the firewall to allow SSH, HTTP and HTTPS traffic.
ufw allow 22
ufw allow 80/tcp
ufw allow 443/tcp
ufw --force enable
...
# variables in the /ect/environment file.
